...
window.addEventListener("scroll", function() {
  ...
  // المسافة التي تم تمريرها حتى الآن
  const scrollTop = window.scrollY || window.pageYOffset;
  // إجمالي ارتفاع الصفحة (بما في ذلك المحتويات المخفية التي تتطلب التمرير للوصول إليها)
  const documentHeight = document.documentElement.scrollHeight;

  // التحقق مما إذا كنا قد وصلنا إلى نهاية الصفحة
  const atBottom = windowHeight + scrollTop >= documentHeight - 1;
if(atBottom && currentPage < lastPage){
  getPostsRequest(false, currentPage + 1);
  currentPage++
  console.log(currentPage);
}
  console.log(atBottom ? "You've reached the bottom of the page!" : "Keep scrolling...");
});
...
function getPostsRequest(reload = true ,page = 1){
  toggleLoader(true);
...
function postClicked(postID) {
  window.location.href = `index1.html?postID=${postID}`;  
  // alert(postID)
...
// get the userclicked id to go to his profile 
function userClicked(userId) {
  window.location.href = `index2.html?userid=${userId}`
}
